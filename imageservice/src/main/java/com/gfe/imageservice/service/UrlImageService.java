...
import com.gfe.imageservice.model.UrlImage;
import com.gfe.imageservice.persistence.ImageRepository;
import com.gfe.imageservice.utils.PaginatedRequest;

@Service
public class UrlImageService implements ImageService{

	Logger log = LoggerFactory.getLogger(UrlImageService.class);
	...
	public Image insert(Image image) {
		UrlImage.Builder builder = new UrlImage.Builder();
		Image imageBD = builder.build();
		try {
			imageBD = imageRepository.insert(image);
	...
	public Image dtoToImage(ImageDto dto) {
		UrlImage.Builder imageBuilder = new UrlImage.Builder();
		UrlImage image = imageBuilder.setId(dto.getId())
										.setName(dto.getName())
										.setDescription(dto.getDescription())
										.setFormat(ImageFormat.getIfPresent(dto.getFormat() != null ? dto.getFormat().toUpperCase() : null))
										.setImageUrl(dto.getUrl())
										.build();
		return image;		
	}
